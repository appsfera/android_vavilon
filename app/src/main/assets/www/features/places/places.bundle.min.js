...
if(Location.isEnabled){Location.getLocation({timeout:10000},!0).then(function(position){$scope.filters.latitude=position.coords.latitude;$scope.filters.longitude=position.coords.longitude},function(error){$scope.filters.latitude=0;$scope.filters.longitude=0}).then(function(){$scope.searchPlaces(!1)})}else{$scope.filters.latitude=0;$scope.filters.longitude=0;$scope.searchPlaces(!1)}})});angular.module('starter').controller('PlacesMapController',function($rootScope,$scope,$ionicSideMenuDelegate,$state,$stateParams,$translate,$timeout,Dialog,Location,Loader,Places,GoogleMaps){angular.extend($scope,{isLoading:!0,isShortLoading:!1,emptyResult:!1,collection:[],showInfoWindow:!1,currentPlace:null,isUserCentered:!1,filters:{e:0,n:0,s:0,w:0,zoom:Places.settings.defaultMapZoom||8},idleTimer:null});window.markers=[];window.markersConfig=[];window.markerClusterer;Places.setValueId($stateParams.value_id);$scope.goToList=function(){$state.go('places-list',{value_id:Places.value_id})};$scope.hideInfoWindow=function(){$scope.showInfoWindow=!1};$scope.$on('$ionicView.enter',function(){$ionicSideMenuDelegate.canDragContent(!1)});$scope.$on('$ionicView.leave',function(){$ionicSideMenuDelegate.canDragContent(!0)});$scope.zoomIn=function(){$scope.crMap.setZoom($scope.crMap.getZoom()+1)};$scope.zoomOut=function(){$scope.crMap.setZoom($scope.crMap.getZoom()-1)};$scope.markerClick=function(marker){$timeout(function(){if(Places.settings.mapAction&&Places.settings.mapAction==='gotoPlace'){$scope.goToPlace(marker.config.place.id)}else{$scope.showInfoWindow=!0;$scope.currentPlace=marker.config.place}})};$scope.loadContent=function(refresh){Places.findAllMaps($scope.filters,refresh).then(function(payload){for(var i=0;i<window.markers.length;i++){window.markers[i].setMap(null)}
window.markers=[];window.markersConfig=[];$scope.collection=payload.places;for(var j=0;j<$scope.collection.length;j++){var place=$scope.collection[j];var marker={config:{id:angular.copy(place.id),place:angular.copy(place)}};if(place.address.latitude&&place.address.longitude){marker.latitude=place.address.latitude;marker.longitude=place.address.longitude}else{marker.address=place.address.address}
switch(place.mapIcon){case 'pin':if(place.pin){marker.icon={url:place.pin,scaledSize:new google.maps.Size(42,42)}}
break;case 'image':if(place.picture){marker.icon={url:place.picture,scaledSize:new google.maps.Size(70,44)}}
break;case 'thumbnail':if(place.thumbnail){marker.icon={url:place.thumbnail,scaledSize:new google.maps.Size(42,42)}}
break;case 'default':default:break}
var tmpMarker=new google.maps.Marker({position:new google.maps.LatLng(marker.latitude,marker.longitude),map:$scope.crMap,index:angular.copy(j),icon:marker.icon});google.maps.event.addListener(tmpMarker,'click',function(){$scope.markerClick(window.markersConfig[this.index])});window.markersConfig.push(angular.copy(marker));window.markers.push(tmpMarker);window.markerClusterer.addMarker(tmpMarker)}}).finally(function(){if($scope.isLoading===!0){$scope.isLoading=!1;Loader.hide()}
$scope.isShortLoading=!1})};$scope.goToPlace=function(placeId){$state.go('places-view',{value_id:Places.value_id,page_id:placeId})};$scope.centerMe=function(startup){if(startup!==undefined&&$scope.isUserCentered===!0){return}
$scope.isUserCentered=!0;Location.getLocation().then(function(position){$scope.crMap.setCenter(new google.maps.LatLng(position.coords.latitude,position.coords.longitude));$scope.crMap.setZoom(Places.settings.defaultCenterZoom||8)},function(){if(startup===undefined){Dialog.alert('Location','Sorry we are unable to locate you, please check your GPS settings & authorization.','OK',-1,'Places')}})};$scope.updateBounds=function(){let jsonBounds=$scope.crMap.getBounds().toJSON();$scope.filters.e=jsonBounds.east;$scope.filters.n=jsonBounds.north;$scope.filters.s=jsonBounds.south;$scope.filters.w=jsonBounds.west;$scope.filters.zoom=$scope.crMap.getZoom()};$scope.initMap=function(){var lat=43.604652;var lng=1.444209;if(Places.settings.hasOwnProperty('lat')&&Places.settings.hasOwnProperty('lng')){lat=parseFloat(Places.settings.lat);lng=parseFloat(Places.settings.lng)}
$scope.mapSettings={zoom:Places.settings.defaultMapZoom||8,center:{lat:lat,lng:lng},cluster:!0,disableDefaultUI:!0,bounds_to_marker:!0};$scope.crMap=GoogleMaps.createMap('crMap',$scope.mapSettings);google.maps.event.addListener($scope.crMap,'idle',function(){$timeout.cancel($scope.idleTimer);if(!$scope.isUserCentered){$scope.centerMe(!0)}
$scope.idleTimer=$timeout(function(){$scope.updateBounds();$scope.isShortLoading=!$scope.isLoading;$scope.loadContent(!0);window.markerClusterer=new MarkerClusterer($scope.crMap,[],{maxZoom:15,minimumClusterSize:2,imagePath:'./img/cluster/m'})},750)})};GoogleMaps.addCallback($scope.initMap);Loader.show($translate.instant('Loading...','places'))});angular.module('starter').controller('PlacesViewController',function($filter,$scope,$rootScope,$state,$stateParams,$translate,$location,Places,SocialSharing,Url){angular.extend($scope,{is_loading:!0,value_id:$stateParams.value_id,social_sharing_active:!1,use_pull_to_refresh:!0,pull_to_refresh:!1,card_design:!1});$scope.blankImage="./features/places/assets/templates/l1/img/blank-700-440.png";Places.setValueId($stateParams.value_id);$scope.loadContent=function(){Places.getPlace($stateParams.page_id).then(function(data){$scope.social_sharing_active=(data.social_sharing_active&&$rootScope.isNativeApp);$scope.blocks=data.blocks;$scope.blockChunks=$filter('chunk')(angular.copy($scope.blocks),Math.ceil($scope.blocks.length/2));$scope.place=data.page;$scope.page_title=data.page_title}).then(function(){$scope.is_loading=!1})};$scope.share=function(){var file;var address="";var link=undefined;angular.forEach($scope.blocks,function(block){if(block.gallery){if(block.gallery.length>0&&file===null){file=block.gallery[0].url}}
...
return $pwaRequest.post('places/mobile_list/find-all-maps',angular.extend({urlParams:{value_id:this.value_id},data:filters,refresh:refresh},factory.extendedOptions))};factory.find=function(placeId){if(!this.value_id){return $pwaRequest.reject('[Factory::Places.find] missing value_id')}
